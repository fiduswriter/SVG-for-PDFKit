var SVGtoPDF = function (doc, svg, x, y, options) {
  ...
  function docEndGroup(group) {
    ...
    group.resources.data.ColorSpace = {...doc.page.colorSpaces};
  ...
  function opacityToColor(color, opacity, isMask) {
    if (typeof color[0] === "string") {
      return [color[0], color[1] * opacity];
    }
  ...
  var SvgElemText = function (obj, inherits) {
    ...
    (function (textParentElem) {
      let processedText = '', remainingText = obj.textContent, textPaths = [], currentChunk = [], currentAnchor,
        currentDirection, currentX = 0, currentY = 0;
